...
The concrete Python code-quality conventions — the lint, typecheck, test-layout, logging, and protocol-conformance rules a change must satisfy. Where `architecture/` governs how code is *structured and designed*, this domain governs the *code-quality details* of the result — consulted both while writing the affected code (picking a log level, adding a Protocol sentinel) and when reviewing whether it is up to standard before it lands.
