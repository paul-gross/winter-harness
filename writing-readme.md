...
- **Installation tells the reader how to install. Period.** Don't list what gets symlinked, what becomes spawnable, or what path notation is unlocked afterward. Effects belong in `index.md` (see `winter-harness:/writing-extension-index.md` for what specifically) or in the file itself — not in Installation.
